...
This version adds to [1.2](1.2.md):

- **[Event-driven creation](#event-driven-creation--event)** — a `generates` action may declare an `event:` and mint the whole document (header and items) when the source reaches a state, at most once, instead of waiting for someone to press a button.
...
- **[`lifecycle`](#lifecycle--the-legal-status-graph)** — the whole legal status graph declared over the status seeds and enforced on **every** status write (user, workflow, glue, transition button), so an unmodeled flip is rejected instead of silently jumping a document between states.
- **[`history`](#history--the-change-trail)** — a shadow change trail per entity: who changed what, from which value to which, when — recorded on the entity's ordinary write path, readable but never writable.
- **[Immutability covers composed collections](#immutablewhen--immutable--user-write-immutability)** — a locked master locks its line items with it, in the API and not only in the UI; [`locksWithMaster`](#lockswithmaster--a-child-collection-that-outlives-its-masters-lock) remains the declared exception.
- **[Resolver-path task assignment](#task-assignment)** — a user task's `assignee:` may walk a to-one path off the trigger entity (`customer.accountManager.username`), validated at parse time, beyond the fixed roles and `personal`.

Version [1.2](1.2.md), [1.1](1.1.md) and [1.0](1.0.md) remain unchanged. Every conforming 1.2
file parses as a conforming 1.3 file; the new constructs are opt-in, with one deliberate
tightening of behaviour: an entity's declared immutability now covers the collections composed
into it, so a generator conforming to 1.3 refuses a user write to a locked document's line items
that a 1.2 generator may have accepted. That was always the lock's intent —
[`locksWithMaster`](#lockswithmaster--a-child-collection-that-outlives-its-masters-lock) is the
declared exception for the collections that must outlive it.
