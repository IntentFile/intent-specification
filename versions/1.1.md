...
This version adds three constructs to 1.0 - [`aggregates`](#aggregates--keyed-cross-entity-totals),
[`checks: kind: guard`](#kind-guard--a-precondition-over-an-aggregate) and
[`posts`](#posts--derived-rows-on-an-event) - and specifies that a derived total is persisted without
overwriting the rest of its row. Version [1.0](1.0.md) remains unchanged. Nothing else is altered:
...
Changing a grouping key on an existing source row moves it between tuples; recomputing the tuple it LEFT is [planned](#planned--recognised-but-not-yet-implemented).
...
- **Aggregate key changes** - editing a grouping key on an existing source row recomputes the tuple it moved INTO; recomputing the tuple it left is not yet specified, so a source whose keys are edited after the fact can leave a stale total behind. Append-only sources (ledgers) are unaffected.
