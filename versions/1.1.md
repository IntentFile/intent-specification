...
This version adds four constructs to 1.0 - [`aggregates`](#aggregates--keyed-cross-entity-totals),
[`checks: kind: guard`](#kind-guard--a-precondition-over-an-aggregate),
[`posts`](#posts--derived-rows-on-an-event) and the reusable
[notify block](#the-notify-block--and-attach-print-sending-the-document-itself) - which sends a message
about a record from a process step, a transition or a schedule, optionally carrying the record's own
rendered document (`attach: print`) and optionally
[fanned out over a related collection](#one-message-per-related-row-foreach) (`forEach`) -
and specifies that a derived total is persisted without
overwriting the rest of its row, that moving a source row between aggregate key-tuples repairs both
tuples, and that a roll-up may maintain a field on a parent another model owns. It also states
[the scope boundary](#the-scope-boundary): what a conforming file deliberately never models, and
what a conforming authoring assistant must say when a requirement falls outside it. Version [1.0](1.0.md) remains unchanged. Nothing else is altered:
a conforming 1.0 file is a conforming 1.1 file.
