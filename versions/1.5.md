...
This version adds to [1.4](1.4.md):

- **[Unrecognised keys are errors](#unrecognised-keys)** — a key the format does not declare is a reported authoring error naming the nearest declared name, never silently dropped; the promise an author writes is either kept or refused, not ignored.
- **[Multilingual report columns](#multilingual-data)** — a report column bound to a translatable property reads in the caller's language, while what a report *matches* stays evaluated against the stored values.
- **[`related`](#related--list-the-records-that-reference-this-entity)** — a read-only register of the records that reference an entity, on its own page: every invoice of this customer, every movement of this product.
- **[`payload`](#payload--the-declared-envelope)** — an outward-facing message may declare its envelope key by key (literals, record fields, one-hop paths, four context tokens) instead of shipping the record as stored.
- **[`outbound`](#outbound--departures-on-a-queue-or-topic)** — the mirror of `inbound`: a record leaves on a queue or topic on an event of the axis, as itself or as its declared payload.
- **[Relative moments in a schedule](#relative-moments--a-where-value-offset-from-now)** — a `where` value may be a moment offset from the firing (`CURRENT_DATE-P7D`), so a staleness sweep needs no hand-written job.
- **[Entity-level `unique`](#unique--a-business-key-over-more-than-one-field)** — a business key over more than one field or to-one relation, enforced by the data store and reported as a distinguishable conflict.

Version [1.4](1.4.md) and its predecessors remain unchanged. The new constructs are opt-in, with
one deliberate tightening of behaviour: an unrecognised key — previously accepted and silently
dropped by a permissive implementation — is now a reported authoring error. A file that only uses
declared keys, in any version, is unaffected; a file that was quietly losing an authored promise now
hears about it.
