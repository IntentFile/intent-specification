...
This version adds to [1.3](1.3.md):

- **[The glue event axis](#the-event-axis--lifecycle-events-and-process-step-events)** — a reacting glue entry (`notifications`, `integrations`) may bind to a **process-step event** (`onStepReached` / `onStepCompleted`), and [`inbound`](#inbound--arrivals-from-outside) gains non-HTTP arrivals: a record from a `queue` or `topic`, or a file dropped in a polled `folder` — every arrival saving through the entity's ordinary write path.
- **[`attach: recordPrint`](#one-document-many-recipients-attach-recordprint)** — one document, many recipients: a `forEach` fan-out attaches the **anchor record's** document, rendered once, to every recipient's message (the request for quotation mailed to each invited supplier), with `{record.<field>}` addressing the anchor.
- **[Notify link placeholders](#links-back-to-the-application-recordurl-inboxurl-appurl)** — `{recordUrl}`, `{inboxUrl}` and `{appUrl}` let a message carry the way back into the application without the intent ever spelling a route.
- **[Role-scoped field visibility](#role-scoped-field-visibility--visibleto)** — `visibleTo:` narrows one field to the callers holding a listed role, enforced where the data leaves the server; the record stays one entity.

Version [1.3](1.3.md) and its predecessors remain unchanged. Every conforming 1.3 file is a
conforming 1.4 file: this version only adds constructs, and every addition is opt-in.
