...
This version, released 2026-09-15, adds to [1.7](1.7.md):

- **[`fromStatus`](#generates--create-from)** — a create-from is guarded by the source's status: the statuses the source may stand in for the action to be offered and accepted, decided by one rule for the button and the endpoint; a create-from declaring only `sourceStatus` is guarded against exactly that status.
- **[`whenMasterDeleted`](#relation-attributes)** — a master's delete and the composition children it owns: `cascade` (the default) deletes the children with the master in one atomic unit, each deletion in full; `refuse` rejects the delete while a child exists, decided before the first cascade touches a row.
- **[`checks: kind: compare`](#kind-compare--two-values-of-one-row)** — two values of one row compared: a field against another of the record's own fields or a typed literal (`ge` / `gt` / `le` / `lt` / `eq` / `ne`), refused with a message on every write or on the transition into a status.
- **[`checks: kind: requiredWhen`](#kind-requiredwhen--a-value-required-only-under-a-condition)** — a value required only under a condition: a field of the record, or a one-hop `Relation.field`, that must be present while a condition over the record's own properties holds.
- **[`items.where` / `items.refuse`](#which-source-rows-become-lines--itemswhere--itemsrefuse)** — which source rows become lines: a mirrored `items` block selects its rows by the rule a schedule's `where` already uses, and an authored refusal turns an unqualified row into a refused generation, decided before the header is built.
- **[`duplicable: { defaults, reset }`](#what-a-copy-does-not-carry-over)** — what a copy does not carry over: the fields a duplicate assigns and the fields it hands back to the entity's own create-time rules.
- **[`retry` / `onError` / `{error}`](#retry--onerror--a-steps-failure-is-part-of-the-model)** — a step's failure is part of the model: a declared retry cycle and an error route on a `delegate:` or a `notify:` service task, with `{error}` recording the final attempt's message on the record.
- **[`personalReadOnly` on a composition](#personal-and-partner-surfaces)** — a composition child may be read-only on a scoped surface independently of its parent: the scope still comes from the parent, the writes do not.
- **[A form field across models](#forms)** — a `relation.field` entry of a form may reach a field of the entity another model owns, read-only, validated against the owner's model rather than the local document.

Three behaviours are stated more precisely rather than added: a [count tile](#dashboard-kpi-widgets) over an
aggregating report shows the report's `count(*)` **summed over its rows**, never the number of groups, and
a report reading through an **optional** to-one relation keeps the records that do not have it; a record
outside every tuple of a [guard](#kind-guard--a-precondition-over-an-aggregate)'s aggregate is **not
guarded**, and an empty `rule.match` literal on a [posting](#postings--source-document-to-ledger) is an
authoring error rather than a lookup for the empty value; and a status a [`processes`](#service-tasks) flow
writes through `setRelationField` is **the flow's column** — a create or update through a generated surface
that sets or changes it is refused as a conflict.

Version [1.7](1.7.md) and its predecessors remain unchanged; the new constructs are opt-in, and the
precision items above remove a latitude implementations had rather than change what a correct one
observably does.
...
> key could ever fire — a fan-out's deliveries are observed per row, not through the step's failure;
...
| [the event axis](#the-event-axis--lifecycle-events-and-process-step-events) | what a reacting glue entry binds to: an entity lifecycle event, a process step reached / completed, or a declared enrichment phase |
...
| [`schedules`](#schedules) | cron: notify or generate records per matching row; `where` values may be moments relative to the firing |
