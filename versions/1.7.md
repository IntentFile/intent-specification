...
This version adds to [1.6](1.6.md):

- **[`immutableInPeriod` / `period`](#immutableinperiod--period--period-locking)** — the second axis of immutability: a register of dated windows with a `closedWhen` condition, and a guarded entity that names the register and the date deciding which window it falls in. User create, update, delete and a move into a closed period are refused; application writes stay possible.
- **[`phases` / `onPhase`](#phases--enrichment-moments-an-entity-announces)** — a moment a silent write announces: an entity declares the enrichment phases it publishes, the enriching listener applies values and announces the phase in one write, and a glue consumer binds `event: { onPhase, phase }` instead of racing the insert.
- **[A roll-up over a foreign child](#a-child-owned-by-another-model--model--parent)** — `rollups.model` / `rollups.parent`: the counted rows are owned by another model and the total lands on a local parent; the reads and the subscription are the owner's, and the dependency edge stays one-way.
- **[`translatable: false`](#keys-not-labels--translatable-false)** — a key of a multilingual entity stays out of the translation table, so what a determination rule matches on, or an arrival looks a relation up by, cannot be translated out from under it.
- **[Field `label:` / `countryLabels:`](#label--countrylabels--a-fields-caption)** — a field says what it is called, and a label fixed by the company's country is resolved from the tenant's country, in every language.

Three behaviours are stated more precisely rather than added: a [roll-up](#rollups--denormalised-parent-totals)'s
status is **relinquished, not only set** — the first move into `statusWhenFull` / `statusWhenPartial`
records the status it displaced, and a sum returning to zero restores it; a
[render](#a-render-reads-in-its-own-language)'s language governs its **data**, not only its template — the
interactive print, a minted copy and a mailed attachment read every translatable value in the language
they are produced in; and a [posting](#an-amended-source-rewrites-its-posting) whose source reaches its
moment **again** with different content **rewrites the post it already made** while that post is still
in the status the posting created it in, instead of silently keeping the first amounts.

Version [1.6](1.6.md) and its predecessors remain unchanged; the new constructs are opt-in, and the
