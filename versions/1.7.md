...
Two behaviours are stated more precisely rather than added: a [roll-up](#rollups--denormalised-parent-totals)'s
status is **relinquished, not only set** — the first move into `statusWhenFull` / `statusWhenPartial`
records the status it displaced, and a sum returning to zero restores it; and a
